...
uint16 Hal_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if(events & HAL_SYS_RESET_EVENT)
  {
    SystemReset();
    return (events ^ HAL_SYS_RESET_EVENT);
  }
  if(events & HAL_FACTORY_RESET_EVENT)
  {
    osal_nv_item_init( ZG_STARTUP_SET, sizeof(startupOptions), (void *)&startupOptions );
    zgWriteStartupOptions( ZG_STARTUP_SET, (ZCD_STARTOPT_DEFAULT_CONFIG_STATE|ZCD_STARTOPT_DEFAULT_NETWORK_STATE) ); 
    SystemReset();
    return (events ^ HAL_FACTORY_RESET_EVENT);
  }
#if (defined HAL_BUZZER) && (HAL_BUZZER == TRUE)
...
void hal_factory_reset( uint32 tim )
{
  if(tim==0)
    osal_set_event(Hal_TaskID,HAL_FACTORY_RESET_EVENT);
  else
    osal_start_timerEx(Hal_TaskID,HAL_FACTORY_RESET_EVENT,tim);
}
void hal_delay_reset(uint32 tim)
{
  if(tim==0)
    osal_set_event(Hal_TaskID,HAL_SYS_RESET_EVENT);
  else
    osal_start_timerEx(Hal_TaskID,HAL_SYS_RESET_EVENT,tim);
}
void hal_stop_delay_reset(void)
{
    osal_clear_event(Hal_TaskID,HAL_SYS_RESET_EVENT);
    osal_stop_timerEx(Hal_TaskID,HAL_SYS_RESET_EVENT);
}
