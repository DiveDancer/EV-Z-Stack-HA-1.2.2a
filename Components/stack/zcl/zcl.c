 ...
 */
static zclInHdlr_t pCoorReceiveCB = NULL;
void zcl_RegisteCoorReceiveCB(uint8 point,zclInHdlr_t cb)
{
  af_registCoordinatorPoint(point);
  pCoorReceiveCB = cb;
}
zclProcMsgStatus_t zcl_ProcessMessageMSG( afIncomingMSGPacket_t *pkt )
{
  ...
  if(pCoorReceiveCB!=NULL)
    pCoorReceiveCB(&inMsg);
